...
### Ethereum data synchronization (optinal)

```sh
docker run -tid crypto_scout:v2.0 /bin/bash
cd ./ethereum/
./execution/geth -datadir ./dataDir -syncmode full --http --http.api eth,net,engine,admin --authrpc.jwtsecret ./consensus/jwt.hex;exec bash
```

open a new terminal

```
docker exex -it [CRYPTO-SCOUT_container_id] /bin/bash
cd ./ethereum/
./consensus/prysm/prysm.sh beacon-chain --checkpoint-sync-url=https://sync-mainnet.beaconcha.in --genesis-beacon-api-url=https://sync-mainnet.beaconcha.in --execution-endpoint=http://localhost:8551 --jwt-secret=./consensus/jwt.hex;exec bash
```
